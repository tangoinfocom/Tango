...
def classificate_request_from_edge(cache_queue, manager_resources_on_cluster,collect_tasks_execute_situation_on_each_node, manager_resources_on_each_node_dict, edge_queue_dict):
    while True:
        ...
        # edge_queue_list[req[3]].put(req)
        # thr = threading.Thread(target=async_BE_threadExecute, args=(req, manager_resources_on_cluster, collect_tasks_execute_situation_on_each_node, manager_resources_on_each_node_dict, myCPU_lock_dict, tmp_write_lock))
        # thr.start()
...
def THR_S_update(LC_S, S_LOCK, s):
    while True:
        ...
        # print("node_service_updatenNum:", node_service_updatenNum)
        with S_LOCK:
...
def lc_schedule_orchetra():
    ...
    # for edgeCluster in all_layout_dict:
        ...
        # client.connect((edge_ip, CLOUD_NODE_LC_LAYOUT_PORT))
...
def THR_collect_data(mission, manager, mdata_lock, managerState):
    ...
    if "BE" in mission:
        ...
        with mdata_lock:
            ...
            # managerState["LC"] = tmp2_dict
        # + str(state['failure']))
    value_dict = valueclone_nested_dict_proxy(managerState)
...
def THR_collect_tasks_execute_situation_on_each_node(pool_args):
    ...
    if success != 0:
        ...
        with task_lock_dict[nodeName][service_id]["ST"]:
            manager_task_dict[masterName][nodeName][service_id]['ST'] += stuck
    # lc_success, lc_failure, be_success, be_failure, lc_stuck, be_stuck = count_task_num(valueclone_nested_dict_proxy(manager_task_dict))
    # logger.info("LC_success：" + str(lc_success) + "  LC_failure：" + str(lc_failure) + "  LC_stuck：" + str(lc_stuck))
    # logger.info("BE_success：" + str(be_success) + "  BE_failure：" + str(be_failure) + "  BE_stuck：" + str(be_stuck))
...
def csv_collect_everthing(manager_resources_on_each_node_dict, manager_resources_on_cluster, manager_task_dict, req_load_dict):
    ...
    # server.bind((CENTRAL_MASTER_IP, CLOUD_REQ_RES_UPDATE_PORT))
    # server.listen(1000)
    # threading.Thread(target=req_center_load, args=(server, record_load)).start()
    ...
    while True:
        ...
        # print("sum_lc_cpu_use:", sum_lc_cpu_use, "  sum_lc_meo_use:", sum_lc_meo_use)
        # print("sum_be_cpu_use:", sum_be_cpu_use, "  sum_be_meo_use:", sum_be_meo_use)
