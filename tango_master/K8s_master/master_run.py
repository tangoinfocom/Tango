...
def run_info_get(service_id, target_node, task_class, MY_PODLIST_DICT):
    ...
    # pod = random.choice([i for i in pod_list])
    # if task_class == "LC":
...
def async_receive_request_from_center(pool_args):
    ...
    if length_data:
        ...
        req = pickle.loads(this_request)
        cache_queue.put(req)
        # this_mission = {req[-2]:{"success": 0, "failure": 0, "stuck": req[-3]}}
        # this_mission = json.dumps(this_mission)
        # this_client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    ...
    with share_lock3:
        # if reqIndex == 10:
            ...
            # with pro_share_lock2:
                # for key,value in NODE_CPU_DICT.items():
                    # sumAvailabel += value
                # avg = sumAvailabel/len(NODE_CPU_DICT)
                # timmReturn = 3/10000*avg*avg-4
                # return 0, 0, timmReturn
...
def async_LC_orchestra_execute(pool_args):
    ...
    if update_num < 0:
        ...
        for i in range(update_num):
            ...
            # logger.info(str(valueclone_nested_dict_proxy(MY_MEO_DICT)))
            if success_get_meo:
...
def LC_orchestra_execute(orchestra_queue, MY_MEO_DICT, pro_share_lock8):
    ...
    while True:
        ...
        # print("get orchestra_req：", orchestra_req)
        if int(orchestra_req["update_num"]) < 0:
            ...
            if tmp_node_service_num <= 1:
                # print("tmp_node_service_num <= 1")
                if orchestra_req in tmp_list:
                    # print("orchestra_req in tmp_list->sleep2.5")
                    time.sleep(2.5)
                    tmp_list.remove(orchestra_req)
                else:
                    # print("orchestra_req not in tmp_list->put back to queue")
...
'''
    state storage API - container info
...
def clear_all(*dict_proxies):
    for dict_proxy in dict_proxies:
        ...
        for key,value in dict_proxy.items():
            ...
            # print("node in dict_proxy and clear")
    ...
    while True:
        if not queue.empty(): 
            ...
            if lc_dict["time_get"] < timeWindow:
                ...
                # with pro_share_lock6:
                    # clear_all(NODE_LC_DICT)
                    # see_lc_dict = valueclone_nested_dict_proxy(NODE_LC_DICT)
                # print("raw_dict_lc:", raw_dict_lc)
                for node in raw_dict_lc:
                    ...
                    for service_type in raw_dict_lc[node]:
                        ...
                        # latency_arr = raw_dict_lc[node][service_type]                                
                        # tailLatency_index = int(len(latency_arr)*0.95)
                        # latency_arr.sort()
                        # print("latency_arr:", latency_arr)
                        # result_lc_latency = latency_arr[tailLatency_index - 1]
                ...
                # with pro_share_lock6:
                    # see_lc_dict = valueclone_nested_dict_proxy(NODE_LC_DICT)
                # print("NODE_LC_DICT:",see_lc_dict)
                # time.sleep(0.1)
        else:
            pass
                # with pro_share_lock6:
...
def THR_decide_LC_movement(share_lock, share_lock2):
    ...
    while True:
        ...
        if tmp_record_canDesicion:
            with share_lock:
                global global_raw_dict_lc
                tmp_record_raw_dict = global_raw_dict_lc.copy()
            desicionSet_DICT = {}
            # for nodeIP in node_IP_List:
                # if nodeIP not in desicionSet_DICT:
                    # continue
                # client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                # client.setblocking(1)
                # client.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                # try:
                    # client.connect((nodeIP, EDGE_NODE_LC_UPDATE_PORT))
                    # client.settimeout(5)
                    # desicionSet = desicionSet_DICT[nodeIP]
                    # LC_desicion = json.dumps(desicionSet)
                    # client.sendall(bytes(LC_desicion.encode('utf-8')))
                    # is_get = client.recv(1024)
                # except Exception as e:
                        # print("THR_decide_LC_movement recv() with error:", e)
                # client.close()
            with share_lock2:
                global_can_desicion_make = False
            tmp_record_canDesicion = False
...
# state storage API - cluster info
def edge_commuicate_info_get(edge_communicate_info_queue):
    ...
    while True:
        ...
        s1.close()
        # raw_data = s1.recv(2048 * 10)
        # edge_communicate_info_queue.put(raw_data)
        # s1.close()
